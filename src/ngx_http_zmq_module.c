...
ngx_http_zmq_handler(ngx_http_request_t *r)
{
  ...
  ngx_chain_t   out;
  ngx_chain_t * c1;
  unsigned char *string, *input, *it;

  /* Get access to the module config variables */
  ngx_http_zmq_loc_conf_t  *zmq_config = ngx_http_get_module_loc_conf(r, ngx_http_zmq_module);
...
/* --------------- BEGIN MAGIC ------------------------- */
  ...
  it = input = ngx_pcalloc(r->pool, r->headers_in.content_length_n+1);
  /* get the message? */
  
  for (c1 = r->request_body->bufs; c1; c1 = c1 -> next)
    it = ngx_copy(it, c1->buf->pos, (c1->buf->last - c1->buf->pos));
#if DEBUG
